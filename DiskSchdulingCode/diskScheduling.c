...
int main() {
	int textQueue[QUEUE_SIZE] = { 98, 183, 37, 122, 14, 124, 65, 67 };

	int localizedQueue[QUEUE_SIZE] = { 30, 32, 35, 33, 40, 37, 38, 36 };

	int randomQueue[QUEUE_SIZE] = { 33, 51, 18, 191, 66, 198, 156, 4 };

	int seekDist[QUEUE_SIZE], scanSeekDist[SCAN_QUEUE_SIZE], seekIdx = 0;
	int head = 53, result = 0;

	printf("\n교과서의 요청 시나리오\n");
	SSTF(textQueue, seekDist, seekIdx, head, result);
	SCAN(textQueue, scanSeekDist, seekIdx, head, result);
	LOOK(textQueue, seekDist, seekIdx, head, result);
...
void SCAN(int* queue, int* seekDist, int seekIdx, int head, int result) {
	...
	printf("Request Queue : ");
	for (int i = 0; i < QUEUE_SIZE; i++) printf("%d\t", reOrder[i]);
	printf("\n");
